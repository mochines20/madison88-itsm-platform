...
const TicketActionPanel = ({ ticket, user, onUpdate }) => {
    ...
    React.useEffect(() => {
        const timer = setInterval(() => {
            const tz = LOCATION_TIMEZONES[ticket.location] || LOCATION_TIMEZONES['Default'];
            try {
    ...
    const handleKbSearch = async (e) => {
        if (e.key === 'Enter') {
            ...
            try {
                ...
                const res = await apiClient.get(`/kb/search?q=${encodeURIComponent(kbQuery)}`);
    ...
    const handleStatusChange = async (newStatus, reason = null, additionalData = {}) => {
        ...
        try {
            setLoading(true);
            const res = await apiClient.patch(`/tickets/${ticket.ticket_id}`, payload);
    ...
    const handleResolutionSubmit = (e) => {
        e.preventDefault();
        handleStatusChange(activeModal === 'close' ? 'Closed' : 'Resolved', 'Issue resolved via modal', {
        ...
        <div className="ticket-action-panel glass">
            ...
            <GlassyModal
                ...
                footer={(
                    <>
                        ...
                        <button className="modal-btn success" onClick={handleResolutionSubmit} disabled={loading || !resolutionForm.root_cause || !resolutionForm.summary}>
            ...
            <GlassyModal
                ...
                footer={(
                    <>
                        <button className="modal-btn secondary" onClick={() => setActiveModal(null)}>CANCEL</button>
                        <button className="modal-btn neutral" onClick={handleResolutionSubmit} disabled={loading || !resolutionForm.root_cause || !resolutionForm.summary}>
                            {loading ? 'CLOSING...' : 'CONFIRM CLOSURE'}
                        </button>
                    </>
                )}
            >
                <div className="modal-info-box warning" style={{ marginBottom: '1.5rem', padding: '12px', borderRadius: '8px', background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.2)', fontSize: '0.9rem', color: '#f59e0b' }}>
                    <strong>Note:</strong> Closing the ticket will archive it and finalize the resolution. Please verify the details below.
                </div>
                <form className="modal-form" onSubmit={handleResolutionSubmit}>
                    <div className="form-field">
                        <label>RESOLUTION CATEGORY</label>
            ...
            <GlassyModal
                ...
                footer={(
                    <>
                        <button className="modal-btn secondary" onClick={() => setActiveModal(null)}>CANCEL</button>
                        <button className="modal-btn primary" onClick={() => handleStatusChange('In Progress', 'Agent took ownership')}>
                            CONFIRM OWNERSHIP
                        </button>
                    </>
                )}
            >
                <p>Are you sure you want to take ownership of this ticket? This will set the status to <strong>IN PROGRESS</strong> and assign it to you.</p>
            </GlassyModal>
            ...
            <GlassyModal
                ...
                footer={(
                    <>
                        <button className="modal-btn secondary" onClick={() => setActiveModal(null)}>CANCEL</button>
                        <button className="modal-btn primary" onClick={handleAssignSubmit} disabled={loading || !selectedStaff}>
                            CONFIRM ASSIGNMENT
                        </button>
                    </>
                )}
            >
                <div className="modal-form">
                    <p style={{ color: '#94a3b8', fontSize: '0.9rem', marginBottom: '1.5rem' }}>Select a staff member to assign this ticket to:</p>
                        ...
                        <div className="dropdown-options-container">
                            {staff
                                .filter(s => {
                                    ...
                                    return s.full_name.toLowerCase().includes(q) || s.location.toLowerCase().includes(q);
                                })
                                .map(member => (
                                    <div
                                        key={member.user_id}
                                    ...
                                    >
                                        ...
                                        <div className="member-details">
                                            ...
                                            <span className="member-meta">
                                                {member.role.replace('_', ' ').toUpperCase()} • {member.location}
                                            </span>
                                        </div>
                    ...
                    <>
                        <div className="panel-header extra-top">
                            <div className="local-time-badge">
                                <span className="label">LOCAL TIME ({ticket.location?.toUpperCase() || 'UTC'})</span>
            ...
            <div className="kb-integration">
                ...
                <div className="kb-search-container">
                    ...
                    <div className="kb-suggestions">
                        {searchingKb && <div className="kb-loader">Searching Knowledge Base...</div>}
                        {!searchingKb && kbResults.length === 0 && kbQuery && <div className="empty-text">No matches found.</div>}
